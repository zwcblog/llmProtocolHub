...
- 输入组织：客户端将文本、图片、音频、视频等内容按平台要求组织为统一消息容器
- 媒体传递：通过 URL、上传后引用、File ID 或内联数据把媒体对象交给模型服务端
- 类型标记：请求体需要明确每个内容块的媒体类型与必要元数据
- 服务端解析：模型服务端按消息结构解析不同模态内容，并决定可接受的组合方式
- 响应生成：模型基于多模态输入返回文本、结构化结果或与上下文相关的后续动作
